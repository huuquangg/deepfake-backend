...
func (r *TransactionRepository) CreateTransaction(tx *models.Transaction) error {
	query := `
		INSERT INTO transactions (transaction_id, from_account_id, to_account_number, amount, description, status, created_at)
...
// GetTransactionsByAccountID gets all transactions for an account
func (r *TransactionRepository) GetTransactionsByAccountID(accountID int) ([]*models.Transaction, error) {
	query := `
		SELECT id, transaction_id, from_account_id, to_account_number, amount, description, status, created_at, completed_at
		FROM transactions
